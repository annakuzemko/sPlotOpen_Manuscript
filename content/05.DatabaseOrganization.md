...
The **'header'** matrix contains plot-level information for the 91,056 vegetation plots provided in sPlot Open, including: metadata (e.g., plot ID, ownership, sampling date, geographical location, positional accuracy); sampling design information (e.g., the total surface area used during the vegetation survey or plot shape); and a plot-level description of vegetation structure (e.g., vegetation type, percentage cover of each vegetation layer) and vegetation type. 
